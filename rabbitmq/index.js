...
class MessageBroker {
    ...
    async sendMessage(channel, queue, requestBody) {
    ...
    async publishMessage(channel, exchange, routingKey, requestBody) {
    ...
    async subscribeMessage(channel, exchange, bindingKey, onSubscribe) {
        await channel.assertExchange(exchange.name, exchange.type, { durable: exchange.durable });
