...
class MessageBroker extends EventEmitter {
    ...
    constructor(MSG_QUEUE_URL, HEARTBEAT_INTERVAL_MS = 5000) {
        ...
        this.on('timeout', (err) => {
            console.error('[MESSAGE-BROKER] Network Timeout: failed to connect with broker server in 10 sec');
        });
     ...
     * This method should be called before initializing channels for workers!
     */ 
    async run() {
        await this.#createConnection();

        console.info("MessageBroker starting...");
    ...
    */
    async #assertQueue(channel, queueDefinition) {
        try {
            ...
            console.error("[MESSAGE-BROKER] #assertQueue Error: ", err.message);
            this.emit('error', err);
        }
    }
    ...
    */
    async publishRpcMessage(channel, exchangeDefinition, replyQueueDefinition, routingKey, payload) {
        try {
            ...
            console.error("[MESSAGE-BROKER] publishRpcMessage error: ", err.message);
            this.emit('error', err);
        }
    }
        ...
        try {
            ...
            channel.consume(declaredQueue, async (msg) => {
                ...
                try {
                    ...
                    console.error("[MESSAGE-BROKER] subscribeRpcMessage Error(sendToQueue): ", err.message);
                    this.emit('error', err);
                }
                channel.ack(msg);
            }, { noAck: false });
        } catch (err) {
            console.error("[MESSAGE-BROKER] subscribeRpcMessage Error: ", err.message);
            this.emit('error', err);
        }
    }
    ...
    */
    async publishToExchange(channel, exchangeDefinition, routingKey, payload, messageProperties = {}) {
        try {
            ...
            //console.info("[SENT] destination exchange : %s, routingKey : %s, msg : %s", exchangeDefinition.name, routingKey, JSON.stringify(payload));
        } catch (err) {
            console.error("[MESSAGE-BROKER] publishToExchange Error: ", err.message);
            this.emit('error', err);
        }
    }
    ...
    */
    async subscribeToExchange(channel, exchangeDefinition, queueDefinition, bindingKey, onDispatch) {
        try {
            ...
            channel.consume(declaredQueue, async function (msg) {
                ...
                if (msg && typeof msg === 'object' && msg.content && msg.fields && msg.properties) {
                    try {
                        ...
                        console.error("[MESSAGE-BROKER] Error executing onDispatch:", error.message);
                        // 에러 발생 시 reject (deadletter로 이동)
                        channel.nack(msg, false, false);
                    }
                } else {
                    // 유효하지 않은 메시지 형식이면 nack하고 deadletter로 이동
                    console.warn("[MESSAGE-BROKER] Invalid message format received:", msg);
                    channel.nack(msg, false, false);
                }
            }, {
                noAck: false,
            ...
            console.error("[MESSAGE-BROKER] subscribeToExchange Error: ", err.message);
            this.emit('error', err);
        }
    };
